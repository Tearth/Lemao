...
pub fn main() -> Result<(), String> {
    ...
    let left_top_text_id = ui.components.store(Label::new(&mut renderer, font_id)?);
    let left_top_text = ui.components.get_and_cast_mut::<Label>(left_top_text_id)?;
    ...
    ui.components.get_mut(ui.main_canvas_id)?.add_child(left_top_text_id);

    let right_top_text_id = ui.components.store(Label::new(&mut renderer, font_id)?);
    let right_top_text = ui.components.get_and_cast_mut::<Label>(right_top_text_id)?;
    ...
    ui.components.get_mut(ui.main_canvas_id)?.add_child(right_top_text_id);

    let left_bottom_text_id = ui.components.store(Label::new(&mut renderer, font_id)?);
    let left_bottom_text = ui.components.get_and_cast_mut::<Label>(left_bottom_text_id)?;
    ...
    ui.components.get_mut(ui.main_canvas_id)?.add_child(left_bottom_text_id);

    let right_bottom_text_id = ui.components.store(Label::new(&mut renderer, font_id)?);
    let right_bottom_text = ui.components.get_and_cast_mut::<Label>(right_bottom_text_id)?;
    ...
    ui.components.get_mut(ui.main_canvas_id)?.add_child(right_bottom_text_id);

    let window_status_text_id = ui.components.store(Label::new(&mut renderer, font_id)?);
    let window_status_text = ui.components.get_and_cast_mut::<Label>(window_status_text_id)?;
    ...
    ui.components.get_mut(ui.main_canvas_id)?.add_child(window_status_text_id);
    ...
    while is_running {
        ...
        if style_changed {
            ui.components.get_and_cast_mut::<Label>(window_status_text_id)?.label_text = format!(
                "Window position: {:?}\nWindow size: {:?}\nWindow style: {:?}\nCursor position: {:?}",
                window.get_position(),
                window.get_size(),
                window.get_style(),
            ...
            ui.components.get_and_cast_mut::<Label>(window_status_text_id)?.dirty = true;
        }

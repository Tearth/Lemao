...
impl Scene<GlobalAppData> for MenuScene {
    fn on_init(&mut self, app: &mut Application<GlobalAppData>) -> Result<(), String> {
        ...
        self.state.ui.logo_panel_id = self.ui.create_panel(&mut app.renderer, ComponentShape::Rectangle)?;
        let logo_panel = self.ui.get_component_and_cast_mut::<Panel>(self.state.ui.logo_panel_id)?;
        ...
        self.ui.get_component_mut(self.ui.main_canvas_id)?.add_child(self.state.ui.logo_panel_id);

        self.state.ui.play_button_id = self.ui.create_button(&mut app.renderer, ComponentShape::Rectangle, font_id)?;
        let play_button = self.ui.get_component_and_cast_mut::<Button>(self.state.ui.play_button_id)?;
        ...
        self.ui.get_component_mut(self.ui.main_canvas_id)?.add_child(self.state.ui.play_button_id);

        self.state.ui.exit_button_id = self.ui.create_button(&mut app.renderer, ComponentShape::Rectangle, font_id)?;
        let exit_button = self.ui.get_component_and_cast_mut::<Button>(self.state.ui.exit_button_id)?;
        ...
        self.ui.get_component_mut(self.ui.main_canvas_id)?.add_child(self.state.ui.exit_button_id);
    ...
    fn on_tick(&mut self, app: &mut Application<GlobalAppData>) -> Result<(), String> {
        ...
        while let Some(event) = self.ui.poll_event() {
            match event {
                UiEvent::ButtonPressed(button_id, _) => {
